...
def engineer_features(df):
    ...
    Ensures all operations are done on 1D Series to avoid shape issues.
    """
    df = df.copy()

    # Ensure 'Close' is a 1D Series
    close = df["Close"]
    # In case it's somehow a DataFrame (multi-column), take first column
    ...
    # Moving averages (using the 1D close series)
