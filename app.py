...
def load_rf_model(mode: str = "daily"):
    ...
    if not os.path.exists(model_path):
        st.error(f"RandomForest model not found: {model_path}. Train and push it first.")
...
def main():
    ...
    if algo.startswith("LSTM") and mode == "hourly":
        st.warning("LSTM is currently only available for DAILY mode.")
        mode = "daily"
        horizon_text = "next day"

    tickers = ["AAPL", "MSFT", "GOOGL", "AMZN", "META", "TSLA", "NVDA", "SPY", "QQQ"]
    ticker = st.selectbox("Ticker", tickers, index=0)

    period = st.selectbox("Historical period (daily)", ["6mo", "1y", "2y", "5y", "10y"], index=2) if mode == "daily" else "60d"

    if st.button("🔍 Run analysis", type="primary"):
        with st.spinner("Downloading data + running model..."):
            yf_interval = "1d" if mode == "daily" else "60m"
            yf_period = period if mode == "daily" else "60d"
            ...
            if algo.startswith("RandomForest"):
                ...
                with tab_signals:
                    st.dataframe(
                        df_feat[["Close", "rsi_14", "pred_up", "prob_up", "pred_return"]].tail(20)
                    )
            ...
            # ---------------- DL ----------------
                ...
                if X_seq.shape[0] == 0:
                    st.error(f"Not enough data to build LSTM window={window}.")
                    return
                ...
                with tab_signals:
                    st.dataframe(
                        df_valid[["Close", "rsi_14", "dl_pred_up", "dl_prob_up", "dl_pred_return"]].tail(20)
                    )

                with tab_model:
                    st.write("LSTM with two heads (direction + magnitude).")
